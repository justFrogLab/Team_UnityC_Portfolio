...
public class PoolManager : MonoBehaviour
{
    ...
    List<GameObject> DropBoxPool = new List<GameObject>();
    int MaxDropBox = 30;
    GameObject DropBoxObj;
    GameObject DropBoxObjGroup;

    [SerializeField] GameObject CircleRange;
    List<GameObject> CircleRangePool = new List<GameObject>();
    int MaxCircleRange = 15;
    GameObject CircleRangeObj;
    GameObject CircleRangeObjGroup;
    ...
    void Awake()
    {
        ...
        #endregion
        #endregion
        DropBoxObjGroup = new GameObject("DropBoxObjGroup");
        for(int i=0;i<MaxDropBox;i++)
        {
            DropBoxObj = Instantiate(DropBox, DropBoxObjGroup.transform);
            DropBoxObj.name = "DropBox" + i.ToString();
            DropBoxObj.SetActive(false);
            DropBoxPool.Add(DropBoxObj);
        }
        CircleRangeObjGroup = new GameObject("CircleRangeObjGroup");
        for (int i = 0; i < MaxCircleRange; i++)
        {
            CircleRangeObj = Instantiate(CircleRange, CircleRangeObjGroup.transform);
            CircleRangeObj.name = "CircleRange" + i.ToString();
            CircleRangeObj.SetActive(false);
            CircleRangePool.Add(CircleRangeObj);
        }
        MissileObjGroup = new GameObject("MissileObjGroup");
        for (int i = 0; i < MaxMissile; i++)
        {
            MissileObj = Instantiate(Missile, MissileObjGroup.transform);
            MissileObj.name = "Missile" + i.ToString();
    ...
    public GameObject GetDropBox()
    {
        ...
        {
            if (DropBoxPool[i].activeSelf == false)//총알 오브젝트 풀이 비활성화 되었다면
            {
                return DropBoxPool[i];//비활성화 된채로 반환
            }
        }
        return null;
    }
    public GameObject GetCircleRange()
    {
        for (int i = 0; i < CircleRangePool.Count; i++)
        {
            if (CircleRangePool[i].activeSelf == false)//총알 오브젝트 풀이 비활성화 되었다면
            {
                return CircleRangePool[i];//비활성화 된채로 반환
            }
        }
        return null;
    }
    public GameObject GetMissile()
    {
        ...
        {
            if (MissilePool[i].activeSelf == false)//총알 오브젝트 풀이 비활성화 되었다면
            {
                return MissilePool[i];//비활성화 된채로 반환
            }
        }
        return null;
    }
    ...
    {//풀링동작함수(몬스터풀                    , 위치저장변수 ,젠포인트배열      ,리젠 시간)
        ...
        {
            ++Transname;
            if (Monster.activeSelf == false&&Monster.GetComponent<ActiveData>().active==false)
            {
    ...
    {//리젠 코루틴 함수     (몬스터 객체       , 해당객체의 젠포인트  , 리젠 시간)
        Monster.GetComponent<ActiveData>().active = true;
        yield return RegenTime;
        Monster.transform.position = ActivePoint.position;
        Monster.SetActive(true);
        Monster.GetComponent<ActiveData>().active = false;
    }
}
